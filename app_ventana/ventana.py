...
def consultar():
    ...
    for elemento in libros:
        ...
        r.insert(INSERT,"\t"+arreglo[0]+"\n Autor:"+arreglo[1]+"\tEditorial:"+arreglo[2]+"\tN° de paginas:"+arreglo[3]+"\tFecha límite:"+arreglo[4]+"\n")
        r.insert(INSERT, "--------------------------------------------------------------------------------\n")
    spinTitulo=Spinbox(ventana, value=(valores), textvariable=eliminarlibro, width=52).place(x=110,y=460)
    r.place(x=30,y=195)
    if libros==[]:
        spinTitulo=Spinbox(ventana, value=(valores), width=52).place(x=110,y=460)
...
eNpaginas=Label(ventana, text="N° de páginas: ", bg=colorFondo, fg=colorLetra).place(x=20,y=130)

cNpaginas=Entry(ventana, textvariable=NdePaginas).place(x=120, y=130)
eFechaLimite=Label(ventana, text="Fecha Límite: ", bg=colorFondo, fg=colorLetra).place(x=20,y=160)

botonAnadir=Button(ventana, text="Añadir libro", command=anadir, bg="#009", fg="white").place(x=550,y=38)

cFechaLimite=Entry(ventana, textvariable=fechalimite).place(x=120,y=160)

spinTitulo=Label(ventana, text="Título leído: ", bg=colorFondo, fg=colorLetra).place(x=20,y=460)

botonLeido= Button(ventana, text="Libro ya leído: ", command=eliminarLibro, bg="#009", fg="white").place(x=550,y=460)

imgbtn=PhotoImage(file="borrar.png")
sal= Button(ventana, image=imgbtn, command=salir).place(x=320,y=75)
